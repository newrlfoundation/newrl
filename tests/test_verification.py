...
def test_create_verification_contract(request):
    ...
    response = requests.post(NODE_URL+'/add-sc', json=
        {
            ...
            "sc_name": "verification",
            "version": "1.0.0",
            "creator": WALLET['address'],
            "actmode": "hybrid",
            "signatories": {"add_verification": None},
            "contractspecs": {
            },
            "legalparams": {}
        }
    ...
    request.config.cache.set('verification_address', ct_address) 



def test_add_verification(request):
    verification_address = request.config.cache.get('verification_address', None)
    wallet = create_wallet()
    fund_wallet_nwrl(WALLET,wallet['address'],8000000)
    asset_id = "sample_id"
    asset_type = "token"
    req_json = {
        "sc_address": verification_address,
        ...
        "params": {
             ...
             "outcome": 1,
             "doc_hash_list": ["doc1","doc2"],
             "doc_link_list": ["doc1","doc2"],
             "remarks":"remarks",
    ...
    params = {
            ...
            'unique_column': "verifier_address",
            'unique_value' : wallet['address']
        }
