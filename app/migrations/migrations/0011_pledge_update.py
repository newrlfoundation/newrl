...
def migrate():
    print("Updating pledge status")
    add_pledge_status_main()


def add_pledge_status_main():
    ...
    add_pledge_status(cur, updates_contract_specs, CUSTODIAN_DAO_ADDRESS)

    con.commit()
    con.close()


def add_pledge_status(cur, contract_specs,address):
    
    try:
        cur.execute(f'''ALTER TABLE pledge_ledger ADD status TEXT''')
    except Exception as e:
        print('Error adding status to pledge', str(e))   
