...
from app.config.ntypes import NEWRL_TOKEN_CODE, NEWRL_TOKEN_DECIMAL, NEWRL_TOKEN_NAME
from app.config.nvalues import ASQI_PID, ASQI_WALLET, FOUNDATION_PID, NETWORK_TRUST_MANAGER_PID, \
    NETWORK_TRUST_MANAGER_WALLET, SENTINEL_NODE_WALLET, ASQI_WALLET_PUBLIC, FOUNDATION_WALLET, FOUNDATION_WALLET_PUBLIC, \
    SENTINEL_NODE_WALLET_PUBLIC, FOUNDATION_TREASURY_ADDRESS, ASQI_TREASURY_ADDRESS, SENTINEL_PID

from app.config.constants import NEWRL_DB


FOUNDATION_RESERVE = 2000000000 * pow(10, NEWRL_TOKEN_DECIMAL)
ASQI_RESERVE = 3000000000 * pow(10, NEWRL_TOKEN_DECIMAL)
...
def init_newrl_tokens():
    ...
    create_person(cur, FOUNDATION_PID)
    link_person_wallet(cur, FOUNDATION_PID, FOUNDATION_WALLET)
    create_person(cur, SENTINEL_PID)
    link_person_wallet(cur, SENTINEL_PID, SENTINEL_NODE_WALLET)
