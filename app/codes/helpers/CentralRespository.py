...
class CentralRepository:
    ...
    def where_clause(self,column,value,operation):
        if self.queryCheck(column) and  self.queryCheck(value):
            self.query=self.query+' where '+column+' '+self.operation[operation]+' '+value
        return self
    def and_clause(self,column,value,operation):
        if self.queryCheck(column) and self.queryCheck(value):
            self.query=self.query+' and '+column+' '+self.operation[operation]+' '+value
        return self
    def or_clause(self,column,value,operation):
        if self.queryCheck(column) and self.queryCheck(value):
            self.query=self.query+' or '+column+' '+self.operation[operation]+' '+value
        return self
    def execute_query_single_result(self):
        return self.cur.execute(self.query).fetchone()
    def execute_query_multiple_result(self):
        return self.cur.execute(self.query).fetchall()
    def execute_query_update(self):
