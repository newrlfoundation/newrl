...
class DaoMainTemplate(ContractMaster):
    ...
    def create_proposal(self, callparamsip,repo:FetchRepository):
        ...
        dao_pid = self.__get_pid_from_wallet_using_repo(repo)
    ...
    def __get_pid_from_wallet_using_repo(self, repo:FetchRepository,address):
        return repo.select_Query("person_id").add_table_name("person_wallet").where_clause("wallet_id", address, 1).execute_query_single_result({"wallet_id":address})
    ...
    def vote_on_proposal(self, callparamsip,repo:FetchRepository):
        ...
        if self.dao_type==2 or self.valid_member(repo, callparams):
            ...
            voter_db_data=repo.select_Query("voter_data,yes_votes,no_votes,abstain_votes,total_votes,function_called").add_table_name("proposal_data").where_clause("proposal_id",proposal_id,1).execute_query_single_result(qparam)
    ...
    def execute(self, callparamsip,repo:FetchRepository):
        ...
        proposal=repo.select_Query("function_called,params").add_table_name("proposal_data").where_clause("proposal_id",callparams['proposal_id']).execute_query_single_result({"proposal_id":callparams['proposal_id']})
    ...
    def add_member(self,callparamsip,repo:FetchRepository):
        ...
        is_dao_exist = repo.select_count().add_table_name("dao_membership").where_clause("dao_person_id",qparam['dao_person_id'],4).and_clause("member_person_id",qparam['member_person_id'],4).execute_query_single_result(qparam)
    ...
    def valid_member(self, repo:FetchRepository, callparamsip):
        ...
        proposal=repo.select_count().add_table_name("dao_membership").where_clause("member_person_id",member_pid,4).execute_query_single_result(qparam)
        proposal=proposal.fetchone()
        if(proposal[0]==0):
            return False
        return True
    ...
    def get_token_lock_amount(self, callparamsip,repo:FetchRepository):
        ...
        lock_data=repo.select_Query("amount_locked").add_table_name("DAO_TOKEN_LOCK").where_clause("person_id",person_id).and_clause("dao_id",dao_id).execute_query_single_result({"person_id":person_id,"dao_id":dao_id})
    ...
    def lock_tokens(self, callparamsip,repo:FetchRepository):
        ...
        # dao_data=cur.execute(f'''Select dao_name as dao_name from dao_main where dao_sc_address=?''',[self.address])
        # dao_data=dao_data.fetchone()
        token_code = callparams['token_code'] #TODO fetch dao name
        # ToDo: Value change
        # transfer_tokens_and_update_balances(
        ...
        lock_data=repo.select_Query("dao_id,person_id,amount_locked").add_table_name("DAO_TOKEN_LOCK").where_clause("person_id",person_id).and_clause("dao_id",dao_id).execute_query_single_result({"person_id":person_id,"dao_id":dao_id})
        if lock_data is None:
            ...
            trxn.append(transaction_creator.transaction_type_two(sc_state_proposal1_data))
            # cur.execute(
