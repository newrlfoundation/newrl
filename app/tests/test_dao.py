...
_dao_manager_add="ct9dc895fe5905dc73a2273e70be077bf3e94ea3b7"
def create_wallet():
...
def create_membership_dao():
    ...
    response = client.post('/call-sc', json={
        {
            "sc_address": _dao_manager_add,
            "function_called": "create",
            "signers": [
                _custodian_wallet['address']
            ],
            "params": {
    ...
    wallets = state['contracts']
    wallet_in_state = next(
        x for x in wallets if x['address'] == ct_address)
    assert wallet_in_state
    return wallet
